...
- [Choose the portable OS-integration boundaries](issues/05-choose-portable-os-integration-boundaries.md) — One `platform_seams.py` module of six functions confines platform difference; Linux child cleanup gains `start_new_session`, `killpg` and `PR_SET_PDEATHSIG` to match the Windows Job Object guarantee; Windows-only engines stay outside the seam; directories stay beside the checkout per ticket 12; and `os.startfile` becomes `open_path()` backed by `xdg-open`.
- [Define the workflow compatibility contract](issues/06-define-workflow-compatibility-contract.md) — Supported means it works when the documented prerequisites are met, with a missing optional prerequisite producing ticket 03's degraded state; every workflow is supported except exact alignment, with video and recording degrading without FFmpeg and tunnelling supported-optional; the LAN endpoint keeps `0.0.0.0` parity and replaces the absent firewall prompt with explicit exposure disclosure.
- [Define Ubuntu support evidence](issues/07-define-support-evidence.md) — Three evidence layers: per-PR `ubuntu-24.04` CI with no Hugging Face traffic (seams, settings migration, downloader against a local fixture server, `requirements.txt` equals `uv export`), scheduled CI proving the real download-and-transcribe path, `windows-latest` CI for the portable Windows invariants plus a manual checklist for the GPU engines, and one clean-VM walk of the documented install ending in an orphan-process check.

## Not yet specified

- None. Every decision on the route to the destination has been made; what remains is implementation, whose sequencing and documentation are handoff work rather than open questions.
